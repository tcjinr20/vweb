...
class Code extends Ctcms_Controller {
	...
	public function index()
	{
		...
		$font_size   = !empty($size) ? $size : 20;
		$img_height  = !empty($h) ? $h : 43;
		$img_width   = !empty($w) ? $w : 100;
		$use_boder   = TRUE;
		$filter_type = 5;
		...
		$_SESSION['codes']=strtolower($rndstring);
	    $_SESSION['authnum_session'] = null;
        setcookie('codes',strtolower($rndstring),1800+time(),Web_Path);
