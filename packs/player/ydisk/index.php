...
function player(){
    $.post("api.php", {"url": "<?php echo get_key($url);?>"},
    function(data){
        if(data['msg'] == 'ok'){
            if(data['ext']=='link'){
                ...
                $('#a1').html('<video id="vod" src="'+data['url']+'" controls="controls" autoplay="autoplay" width="100%" height="100%"></video>');
				isiPad = true;
				setInterval('error()',1000);
            }else{
                if(data['ext']=='m3u8' || data['ext']=='m3u8_list'){
                    var flashvars={f:'/packs/player/ckplayer/m3u8.swf',a:data['url'],c:0,s:4,lv:0,p:1,v:100}
